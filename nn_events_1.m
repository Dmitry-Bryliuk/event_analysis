...
%   похожих событий
%
% что ещё надо сделать из главного:
% - внятные подписи и позиционирование графиков, пока что не понять
%   что оно показывает
% всякие промежуточные технические моменты:
% - сохранение промежуточных результатов парсинга, чтобы
...
%{
текущая обстановка сейчас такая:

- c нейронной сетью порядок, обучается, распознаёт и даже кое-как
  сканирует. для улучшения результатов можно попробовать следующее:
  - поменять параметры обучения, тип сети или алгоритм обучения. может быть
    количество нейронов и слоёв.
  - конструирование обучающей выборки (см. далее)
  - преобразование исходных событий в другой вид (спектр, разности, ...)
  - форма и ширина всплеска события

- сканирование шкалы - в окно попадают события, которые явно за пределами
  как диапазона событий по всем классам, так и по отдельным, что может
  мешать классификации. что здесь можно сделать:
  - обнулять при выборке из основной шкалы всё что за пределами
    максимального диапазона дат классов
  - подача окна с занулением всего что за пределами дат конкретного
    класса, итерация по каждому классу и выбор лучшего значения
  - отдельная нейронная сеть на каждый класс
  - возможно подшаманить с обучающими примерами, сделать много случайных
    вариаций за пределами целевого диапазона, в т.ч. подача единиц-нулей по
    комбинациям факторов вне целевого диапазона

- другие вещи, которые желательно попробовать:
  - нулевые и единичные отрицательные примеры
  - проверить детектирование на неискажённых дубликатах
  - искажение/зашумление всей шкалы и отдельных примеров, чтобы проверить
    насколько всё чётко работает?

- ещё одна интересная идея - подавать на вход нс не дискретную шкалу, а
  смещения факторов в окне друг относительно друга (возможно с
  дополнительной разбивкой на иерархические перекрывающиеся окна внутри
  главного окна, по типу свёрточной сети). но есть нюансы, надо думать.
  если получится, всё становится много проще и частотные преобразования
...
if nn_trained
    ...
    % параметры разделения обучающей выборки
    % кидаем всё на обучение
    % это была наша главная проблема, при единичных правильных примерах на
    % класс они часто даже не попадали в обучающую выборку
    ...
    if nn_classify_error_total > 0
        nn_error_message = ' (большая погрешность, поменяйте параметры!)';
        disp('   !!!   большая погрешность нейронной сети, поменяйте параметры обучения   !!!');
    end

    figure('Name', ['классификация и ошибки нейронной сети на обучающих примерах' nn_error_message]);
    subplot(3, 1, 1);
...
for event_date_original = nn_all_step_positions_original
    ...
    [nn_window_result, nn_event_window_position_start, window_event_matrix, out_of_range] = calc_nn_window(event_date_original);
    if out_of_range
        fprintf('- достигнут предел сканирования\n');
        break;
    end
    nn_event_window_position_start = nn_event_window_position_start + event_wave_window_size_scaled_half;
    nn_all_window_result(nn_event_window_position_start:nn_event_window_position_start+time_line_zoom-1, :) = repmat(nn_window_result, 1, time_line_zoom)';
