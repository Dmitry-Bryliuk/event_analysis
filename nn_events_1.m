...
- нейронная сеть после обучения не умеет распознавать даже обучающие
  примеры. это главная проблема, с этим надо в первую очередь разобраться.
  - скорее всего надо поменять параметры обучения, тип сети или алгоритм
    обучения. количество нейронов и слоёв вроде как не влияет.
  - возможно плохая обучающая выборка, но это вряд ли.
