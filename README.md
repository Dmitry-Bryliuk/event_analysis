...
результат сканирования скользящим окном нейронной сетью, уже что-то показывает. но пока бардак, т.к. нейронная сеть не научилась классифицировать даже обучающие примеры, надо разбираться с параметрами обучения. всплески должны показывать где нс обнаружила похожие группы событий. первые два класса есть в исходной файле, десять других классов сгенерированы случайно.
...
склонировать репозиторий локально на комп, в командной строке: `git clone git@github.com:Dmitry-Bryliuk/event_analysis.git`
