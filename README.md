...
- главный скрипт с анализом нейронными сетями:
  ...
  - что уже умеет:
    ...
    - обучение нейронной сети исходными и сгенерированными примерами
    - сканирование всей шкалы скользящим окном нейронной сетью и графики отклика по классам (предварительно, ещё допиливать нормальный вывод и проверять что получилось)
...
возможно предобработка делается много проще с помощью соответствующих тулбоксов, но это надо разбираться отдельно (в том смысле что непонятно даже куда именно смотреть)
...
линейные матрицы событий по классам (графики факторов сцеплены последовательно в одну линию, чтобы подать на вход нс):

![линейные матрицы событий по классам](../master/pictures/readme/events_by_classes_linear_1.png "линейные матрицы событий по классам")

результат сканирования скользящим окном нейронной сетью, уже что-то показывает. для улучшения результатов надо поэкспериментировать с параметрами обучения нс, обучающей выборкой, преобразованием входных данных. всплески должны показывать где нс обнаружила похожие группы событий. первые два класса есть в исходной файле, десять других классов сгенерированы случайно.

![nn_window_scan_1.png](../master/pictures/readme/nn_window_scan_1.png)
...
склонировать репозиторий локально на комп, в командной строке: `git clone git@github.com:имя_пользователя/event_analysis.git` (точную ссылку см. справа вверху по кнопке "Clone or download")

или используем git extensions, правой кнопкой на каталоге в проводнике, выбираем git clone и копипастим адрес репозитории.

на отдельном файле отмечаем git commit чтобы изменения ушли в репозиторий, и командой git push отмеченные файлы записываются в репозиторий. git clean - откатить изменения.

похожим способом работает Visual Studio Code, только там надо отмечать файлы как stage из вкладки git, потом commit, потом уже push из меню.

если репозиторий обновился, то локально его обновить командой git pull.
